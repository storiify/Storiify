...
<div class="conteudo">
    ...
    <div class='pos-cabecalho mx-auto'>
        <a href='?categoria=<?= $categoria ?>&acao=cadastrar' 
           title='Clique para criar uma nova <?= ModeloFlora::$nomeSingular ?>'
           class='btn btn-azul criar-nova-instancia'>
            <i class="fa fa-plus"></i>
            &nbsp&nbspCriar nova <?= ModeloFlora::$nomeSingular ?>
        </a>
    ...
    if (isset($resultados)) {
        foreach ($resultados as $modeloArray) {
            $flora = new ModeloFlora($modeloArray);
            $nome = ($flora->nm_flra());

            $camposListar = "";
            foreach ($flora->getAtributosListar() as $nomeCampo => $valorCampo) {
                $camposListar .= "<tr>"
            ...
            . "class='instancia-card mx-auto row clicavel' data-id='{$flora->pk_flra()}' data-nome='$nome'>"
            ...
            . "        </div>"
            . "    </div>"
            . "    <a href='?categoria=$categoria&acao=editar&id={$flora->pk_flra()}'></a>"
            ...
            . "        </button>"
            . "        <a href='?categoria=$categoria&acao=excluir&id={$flora->pk_flra()}' class='deletar-instancia-escondido'></a>"
    ...
    if (!isset($flora)) {
        ...
        . "	    <span>" . sprintf(const_Indefinida_Msg, ModeloFlora::$nomePlural) . "</span>"
        . "	</div>"
