...
<div id="titulo-bg">
    <div id="categoria-titulo" class="row">
        <h1><?= ModeloReligiao::$nomePlural ?></h1>
    </div>    
</div>
...
<div class="conteudo">
    ...
    <div class='pos-cabecalho mx-auto'>
        <a href='?categoria=<?= $categoria ?>&acao=cadastrar' 
           title='Clique para criar uma nova <?= ModeloReligiao::$nomeSingular ?>'
           class='btn btn-azul criar-nova-instancia'>
            <i class="fa fa-plus"></i>
            &nbsp&nbspCriar nova <?= ModeloReligiao::$nomeSingular ?>
        </a>
    ...
    if (isset($resultados)) {
        foreach ($resultados as $modeloArray) {
            $religiao = new ModeloReligiao($modeloArray);
            $nome = ($religiao->nm_relg());

            $camposListar = "";
            foreach ($religiao->getAtributosListar() as $nomeCampo => $valorCampo) {
                $camposListar .= "<tr>"
            ...
            . "class='instancia-card mx-auto row clicavel' data-id='{$religiao->pk_relg()}' data-nome='$nome'>"
            ...
            . "        </div>"
            . "    </div>"
            . "    <a href='?categoria=$categoria&acao=editar&id={$religiao->pk_relg()}'></a>"
            ...
            . "        </button>"
            . "        <a href='?categoria=$categoria&acao=excluir&id={$religiao->pk_relg()}' class='deletar-instancia-escondido'></a>"
    ...
    if (!isset($religiao)) {
        ...
        . "	    <span>" . "Seus " . ModeloReligiao::$nomePlural . " aparecerão aqui" . "</span>"
        . "	</div>"
