...
<div class="conteudo">
    ...
    <div class='pos-cabecalho mx-auto'>
        <a href='?categoria=<?= $categoria ?>&acao=cadastrar' 
           title='Clique para criar uma nova <?= ModeloRecurso_natural::$nomeSingular ?>'
    ...
    if (isset($resultados)) {
        foreach ($resultados as $modeloArray) {
            $rcsNat = new ModeloRecurso_natural($modeloArray);
            $nome = ($rcsNat->nm_rcs_ntrl());

            $camposListar = "";
            foreach ($rcsNat->getAtributosListar() as $nomeCampo => $valorCampo) {
                $camposListar .= "<tr>"
            ...
            . "class='instancia-card mx-auto row clicavel' data-id='{$rcsNat->pk_rcs_ntrl()}' data-nome='$nome'>"
            ...
            . "        </div>"
            . "    </div>"
            . "    <a href='?categoria=$categoria&acao=editar&id={$rcsNat->pk_rcs_ntrl()}'></a>"
            ...
            . "        </button>"
            . "        <a href='?categoria=$categoria&acao=excluir&id={$rcsNat->pk_rcs_ntrl()}' class='deletar-instancia-escondido'></a>"
    ...
    if (!isset($rcsNat)) {
        ...
        . "	    <span>" . sprintf(const_Indefinida_Msg, ModeloRecurso_natural::$nomePlural) . "</span>"
        . "	</div>"
