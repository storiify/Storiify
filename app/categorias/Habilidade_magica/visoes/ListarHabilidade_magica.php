...
<div class="conteudo">
    ...
    <div class='pos-cabecalho mx-auto'>
        <a href='?categoria=<?= $categoria ?>&acao=cadastrar' 
           title='Clique para criar uma nova <?= ModeloHabilidade_magica::$nomeSingular ?>'
           class='btn btn-azul criar-nova-instancia'>
            <i class="fa fa-plus"></i>
            &nbsp&nbspCriar nova <?= ModeloHabilidade_magica::$nomeSingular ?>
        </a>
    </div>

    <?php
    if (isset($resultados)) {
        foreach ($resultados as $magicaArray) {
            $magica = new ModeloHabilidade_magica($magicaArray);
            $nome = ($magica->nm_hbld_mgca());

            $camposListar = "";
            foreach ($magica->getAtributosListar() as $nomeCampo => $valorCampo) {
                $camposListar .= "<tr>"
            ...
            . "class='instancia-card mx-auto row clicavel' data-id='{$magica->pk_hbld_mgca()}' data-nome='$nome'>"
            ...
            . "        </div>"
            . "    </div>"
            . "    <a href='?categoria=$categoria&acao=editar&id={$magica->pk_hbld_mgca()}'></a>"
            ...
            . "        </button>"
            . "        <a href='?categoria=$categoria&acao=excluir&id={$magica->pk_hbld_mgca()}' class='deletar-instancia-escondido'></a>"
    ...
    if (!isset($magica)) {
        ...
        . "	    <span>" . sprintf(const_Indefinida_Msg, ModeloHabilidade_magica::$nomePlural) . "</span>"
        . "	</div>"
