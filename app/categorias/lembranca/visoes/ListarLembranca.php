...
<div class="conteudo">
    ...
    <div class='pos-cabecalho mx-auto'>
        <a href='?categoria=<?= $categoria ?>&acao=cadastrar' 
           title='Clique para criar uma nova <?= ModeloLembranca::$nomeSingular ?>'
           class='btn btn-azul criar-nova-instancia'>
            <i class="fa fa-plus"></i>
            &nbsp&nbspCriar nova <?= ModeloLembranca::$nomeSingular ?>
        </a>
    </div>

    <?php
    if (isset($resultados)) {
        foreach ($resultados as $lembrancaArray) {
            $lembranca = new ModeloLembranca($lembrancaArray);
            $nome = ($lembranca->dcr_lmca());

            $camposListar = "";
            foreach ($lembranca->getAtributosListar() as $nomeCampo => $valorCampo) {
                $camposListar .= "<tr>"
            ...
            . "class='instancia-card mx-auto row clicavel' data-id='{$lembranca->pk_lmca()}' data-nome='$nome'>"
            ...
            . "        </div>"
            . "    </div>"
            . "    <a href='?categoria=$categoria&acao=editar&id={$lembranca->pk_lmca()}'></a>"
            ...
            . "        </button>"
            . "        <a href='?categoria=$categoria&acao=excluir&id={$lembranca->pk_lmca()}' class='deletar-instancia-escondido'></a>"
    ...
    if (!isset($lembranca)) {
        ...
        . "	    <span>" . sprintf(const_Indefinida_Msg, ModeloLembranca::$nomePlural) . "</span>"
        . "	</div>"
