...
class BdContextLembranca extends ConexaoBd {
    ...
    public function salvar($parametros) {
        ...
        if ($parametros['pk_lmca'] != '') { //Ao editar
            $condicao = " pk_lmca='{$parametros['pk_lmca']}'";
            $res = $this->updateBase($parametros, $this->tabela, $condicao);
            consoleLog("BdContext: " . $res);
        } else { //Ao criar
            unset($parametros['pk_lmca']);
    ...
    public function personagensQueAparece($parametros) {
        $tbPsnaLmca = "tb_personagem_rel_Lembranca";
        $idPersonagem = $parametros;
        $condicao = "WHERE fk_lmca='$idPersonagem'";

        $res = $this->listarBase('fk_psna', $tbPsnaLmca, $condicao);
